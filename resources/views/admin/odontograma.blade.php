...
@section('css')
    <script src={{ asset('assets/odontograma/js/jquery-1.7.2.min.js') }}></script>
    <link type="text/css" href="{{ asset('assets/odontograma/css/custom-theme/jquery-ui-1.8.13.custom.css') }}"
        rel="stylesheet" />
    <script type="text/javascript" src="{{ asset('assets/odontograma/js/jquery-ui-1.8.13.custom.min.js') }}"></script>
    <style>
        #myCanvas {
...
@section('content')
    ...
    <div id="radio">
        <input type="radio" id="radio1" name="accion" value="fractura" checked="checked" /><label
            for="radio1">Fractura</label>
        <input type="radio" id="radio2" name="accion" value="restauracion" /><label
            for="radio2">Restauracion</label>
            ...
            <div class="col-md-12">
                <div class="card">
                    <div class="card-header">
                        <h1 class="card-title">Odontograma</h1>
                    </div>
                    <div class="card-body">
                        <form action="{{ route('odontograma.store') }}" method="POST">
                            ...
                            <div class="row">
                                <!-- Campo para el número de diente -->
                                <div class="col-md-4 mb-3">
                                    <label for="numeroDiente" class="form-label">Número de Diente:</label>
                                    <input type="number" name="numeroDiente" id="numeroDiente" class="form-control"
                                        required readonly>
                                </div>

                                <!-- Campo de solo lectura para color -->
                                <div class="col-md-4 mb-3">
                                    <label for="color" class="form-label">Estado:</label>
                                    <input type="text" name="estadoDiente" id="color" class="form-control">
                                </div>

                                <!-- Campo de solo lectura para sección -->
                                <div class="col-md-4 mb-3">
                                    <label for="seccionDiente" class="form-label">Sección:</label>
                                    <input type="text" name="seccionDiente" id="seccionDiente" class="form-control">
                                </div>
                    ...
                    <button type="submit" class="btn btn-primary">Guardar Odontograma</button>
                    </form>
                </div>
            </div>
        </div>
...
@section('js')
    ...
    <script>
        ...
        // Funcion para sombrear diente completo "puente"
        ...
        function marcar_puente(contexto, dient_1, dient_2, color_pas) {
            ...
            inicio_x =
                num_diente1 * med +
                separacion_x * num_diente1 +
                separacion_x +
                med / 2;
            fin_x =
                num_diente2 * med +
                separacion_x * num_diente2 +
                separacion_x +
                med / 2;
            ctx.fillStyle = color_line;
            ctx.beginPath();
        ...
        function getPosition(event) {
            ...
            if (diente) {
                //alert(diente);
                // Asignar el valor del diente al campo de número de diente
                document.getElementById('numeroDiente').value = diente;
                ...
                if (accion == "seccion") {
                    ...
                    if (diente1 == 0) {
                        ...
                        diente2 = mayor;
                        if (
                            (diente1 < 17 && diente2 < 17 && diente1 != diente2) ||
                            (diente1 > 17 && diente2 > 17 && diente1 != diente2)
                        ) {
                            marcar_diente(ctx4, diente, "red");
                    ...
                    if (seccion_chk == "seccion") {
                        ...
                        if (seccion_b) {
                            ...
                            for (var i = 0; i < localStorage.length; i++) {
                                ...
                                if (
                                    diente_comp == diente &&
                                    seccion_b == seccion_comp &&
                                    (accion_comp == 1 || accion_comp == 2)
                                ) {
                                    if (ultimo == "") {
                                        ultimo = item;
                                        key_cod = key_name;
            ...
            if (seccion && color != "") {
                ...
                document.getElementById('seccionDiente').value = seccion;
                console.log("Color seleccionado: " + color);
                // Asignar el valor del estado del diente al campo de color
                var estadoDiente;

                // Mapeo de colores a estados de dientes
                var colorEstadoMap = {
                    'red': 'f', // Fractura
                    'blue': 'r', // Restauración
                    'black': 'e', // Extracción
                ...
                // Asignar el valor de acuerdo al mapeo
                estadoDiente = colorEstadoMap[color] || '';

                // Asignar el valor al campo de solo lectura
                document.getElementById('color').value = estadoDiente;
                //alert(color);
